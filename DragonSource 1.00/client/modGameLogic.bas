...
Sub MenuState(ByVal State As Long)
    ...
    If Not IsConnected Then
        frmMainMenu.Visible = True
        frmSendGetData.Visible = False
        Call MsgBox("Sorry, the server seems to be down.  Please try to reconnect in a few minutes or visit http://source.draignet.com/", vbOKOnly, GAME_NAME)
    End If
End Sub
...
Dim x1, y1 As Long
    ...
    If InEditor Then
        ...
        If (Button = 1) And (x1 >= 0) And (x1 <= MAX_MAPX) And (y1 >= 0) And (y1 <= MAX_MAPY) Then
            If frmMain.optLayers.Value = True Then
                With Map.Tile(x1, y1)
                    If frmMain.optGround.Value = True Then .Ground = EditorTileY * 7 + EditorTileX
                    If frmMain.optMask.Value = True Then .Mask = EditorTileY * 7 + EditorTileX
                    If frmMain.optAnim.Value = True Then .Anim = EditorTileY * 7 + EditorTileX
                    If frmMain.optFringe.Value = True Then .Fringe = EditorTileY * 7 + EditorTileX
                End With
            Else
                With Map.Tile(x1, y1)
                    If frmMain.optBlocked.Value = True Then .Type = TILE_TYPE_BLOCKED
                    If frmMain.optWarp.Value = True Then
                        .Type = TILE_TYPE_WARP
                        .Data1 = EditorWarpMap
                        .Data2 = EditorWarpX
                        .Data3 = EditorWarpY
                    End If
                    If frmMain.optItem.Value = True Then
                        .Type = TILE_TYPE_ITEM
                        .Data1 = ItemEditorNum
                        .Data2 = ItemEditorValue
                        .Data3 = 0
                    End If
                    If frmMain.optNpcAvoid.Value = True Then
                        .Type = TILE_TYPE_NPCAVOID
                        .Data1 = 0
                        .Data2 = 0
                        .Data3 = 0
                    End If
                    If frmMain.optKey.Value = True Then
                        .Type = TILE_TYPE_KEY
                        .Data1 = KeyEditorNum
                        .Data2 = KeyEditorTake
                        .Data3 = 0
                    End If
                    If frmMain.optKeyOpen.Value = True Then
        ...
        If (Button = 2) And (x1 >= 0) And (x1 <= MAX_MAPX) And (y1 >= 0) And (y1 <= MAX_MAPY) Then
            If frmMain.optLayers.Value = True Then
                With Map.Tile(x1, y1)
                    If frmMain.optGround.Value = True Then .Ground = 0
                    If frmMain.optMask.Value = True Then .Mask = 0
                    If frmMain.optAnim.Value = True Then .Anim = 0
                    If frmMain.optFringe.Value = True Then .Fringe = 0
                End With
            Else
                With Map.Tile(x1, y1)
                    .Type = 0
...
Public Sub EditorTileScroll()
    frmMain.picBackSelect.top = (frmMain.scrlPicture.Value * PIC_Y) * -1
End Sub
...
Dim YesNo As Long, x As Long, y As Long
    ...
    ' Ground layer
    If frmMain.optGround.Value = True Then
    ...
    ' Mask layer
    If frmMain.optMask.Value = True Then
    ...
    ' Animation layer
    If frmMain.optAnim.Value = True Then
    ...
    ' Fringe layer
    If frmMain.optFringe.Value = True Then
...
Sub ResizeGUI()
    If frmMain.WindowState <> vbMinimized Then
        frmMain.txtChat.Height = Int(frmMain.Height / Screen.TwipsPerPixelY) - frmMain.txtChat.top - 32
        frmMain.txtChat.Width = Int(frmMain.Width / Screen.TwipsPerPixelX) - 8
    End If
End Sub
